    ...
    <ThemedView className="flex-1">
      <View className="px-5 py-3 border-b border-gray-100 dark:border-gray-800">
        <View className="flex-row gap-2 mb-3">
          ...
          <StatBadge label="En Manto." value={maintenanceCount} color="#F59E0B" />
          <StatBadge label="Disponible" value={rooms?.filter((r) => r.room_status === "available").length || 0} color="#10B981" />
        </View>
        ...
        <View className="flex-row bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
          ...
          >
            <ThemedText className={`text-sm font-semibold ${filter === "all" ? "text-[#0EA5E9]" : "opacity-60"}`}>Todas</ThemedText>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <ThemedText className={`text-sm font-semibold ${filter === "maintenance" ? "text-[#0EA5E9]" : "opacity-60"}`}>Mantenimiento ({maintenanceCount})</ThemedText>
          </TouchableOpacity>
        </View>
      </View>
